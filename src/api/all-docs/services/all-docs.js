  ...
  allDocs : async () => {
    try {
      const marcoLegal = await strapi.entityService.findMany(
          "api::marco-legal.marco-legal", docsQuery);
      const projects = await strapi.entityService.findMany(
          "api::proyecto.proyecto", docsQuery);
      ...
      const formatosConvenios = await strapi.entityService.findMany(
          "api::formato-convenio.formato-convenio", docsQuery);
      const educacionContinua = await strapi.entityService.findMany(
          "api::educacion-continua.educacion-continua", docsQuery);
      const practicasPreprofesionales = await strapi.entityService.findMany(
          "api::practica-preprofesional.practica-preprofesional", docsQuery);

      const resolucionAprobaciones = await strapi.entityService.findMany(
          "api::resolucion-aprobacion.resolucion-aprobacion", docsQuery);
      ...
      const comercioJusto = await strapi.entityService.findMany(
          "api::comercio-justo.comercio-justo", docsQuery);
      const auditorio = await strapi.entityService.findMany(
          "api::auditorio.auditorio", docsQuery);
      ...
      const encuentros = await strapi.entityService.findMany(
          "api::encuentro-de-graduados.encuentro-de-graduados", docsQuery);
      return DocsReducedData(marcoLegal)
          .concat(projects, formatos, externalBudget, informesSeguimiento,
                  resolucionAprobaciones, conveniosInterinstitucionales,
                  formatosConvenios, educacionContinua, informesCapacitacion,
                  practicasPreprofesionales, gestion, comercioJusto, auditorio,
                  informesInsercion, cursos, informes, formatosSeg, encuentros);
    } catch (err) {
      return err;
    }
  },
